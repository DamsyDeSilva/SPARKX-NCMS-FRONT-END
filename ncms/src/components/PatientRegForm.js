...
export default function PatientRegForm() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const regData = {firstName, lastName, gender, district, locationX, locationY,};
    console.log(regData);
  };
  ...
  return (
    <div className={patientRegForm}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <select className={forminput} required value={gender} onChange={(e) => setGender(e.target.value)}>
            <option value="Male">Male</option>
            <option value="Female">Female</option>
        </select>

        <label className={regLabel}>District : </label>
        <select className={forminput} required value={district} onChange={(e) => setDistrict(e.target.value)}>
